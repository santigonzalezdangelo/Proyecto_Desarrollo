...
class ReservationController {
  create = async (req, res) => {
    try {
      ...
      const reserva = await reservationDao.create({ id_usuario, id_propiedad, fecha_inicio, fecha_fin });
      return res.status(200).json(reserva); 
    } catch (err) {
      console.error("Error creando reserva:", err);
      return res.status(500).json({ error: err?.message ?? "Error creando reserva" });
    }
  };

  getMine = async (req, res) => {
    try {
      const raw = req.query.id_usuario ?? req.params.id_usuario ?? req.user?.id_usuario;
      ...
      const mapped = rows.map(r => {
        ...
        const etiqueta = now < d0 ? "proxima" : (now < d1 ? "activa" : "finalizada");
