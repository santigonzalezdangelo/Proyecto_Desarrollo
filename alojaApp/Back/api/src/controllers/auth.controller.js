...
class AuthController {
  ...
  // === PATCH: firmar con signToken y setear cookie con setAuthCookie ===
  login = async (req, res) => {
    try {
      const { correo, password } = req.body ?? {};
      if (!correo || !password) {
      ...
      // 🔐 FIRMA UNIFICADA (usa process.env.JWT_SECRET dentro del util)
      // Incluyo ambas keys por compatibilidad con tu requireAuth (id / id_usuario)
      ...
      // 🍪 Setea la cookie con flags correctos (mismo nombre que lee requireAuth)
