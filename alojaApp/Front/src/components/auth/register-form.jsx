...
import { useState } from "react";
import { Mail, Lock, Eye, EyeOff, ChevronLeft, User, Fingerprint } from "lucide-react";
import { useNavigate, Link } from "react-router-dom";
...
export default function Register() {
  ...
  const DEFAULT_ROLE_ID = import.meta.env.VITE_DEFAULT_ROLE_ID;

  const onChange = (e) => {
    const { name, value } = e.target;
    const v = name === "dni" ? value.replace(/\D/g, "") : value;
    setForm((f) => ({ ...f, [name]: v }));
  };

  const validEmail = (v) => /\S+@\S+\.\S+/.test(v);
  const validName  = (v) => String(v).trim().length >= 2;
  const validDni   = (v) => /^\d{7,9}$/.test(v);
  const passOk     = form.password.length >= 8;
  const same       = form.password === form.confirm;

  const canSubmit =
    validName(form.nombre) &&
    validName(form.apellido) &&
    validDni(form.dni) &&
  ...
  return (
    <div
      className="min-h-svh bg-neutral-50 flex justify-center items-start md:items-center p-4 md:p-6"
      style={{ paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}
    >
      <div className="relative w-full max-w-2xl">

        <div className="bg-white rounded-3xl shadow-2xl border border-neutral-100 p-6 md:p-8">
          <div className="flex flex-col items-center mb-6">
            <img
              src="/images/logo.png"
              alt="AlojaApp"
              ...
              style={{ maxHeight: 44 }}
            />
            <h1 className="mt-3 text-2xl md:text-3xl font-bold tracking-tight text-center">
              Crear cuenta
            </h1>
            <p className="mt-1 text-sm text-neutral-600 text-center">
              Registrate para gestionar reservas o publicar propiedades.
          ...
          <form onSubmit={submit} className="space-y-4">
            {/* 2 columnas desde md: Nombre | DNI  /  Apellido | Correo */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Nombre */}
              <div>
                <label className="block text-sm font-medium text-neutral-800 mb-1">Nombre</label>
                <div className="flex items-center gap-2 rounded-xl border border-neutral-300 bg-white px-3 py-2 focus-within:ring-2 ring-amber-300">
              ...
              {/* DNI */}
              <div>
                <label className="block text-sm font-medium text-neutral-800 mb-1">DNI</label>
                <div className="flex items-center gap-2 rounded-xl border border-neutral-300 bg-white px-3 py-2 focus-within:ring-2 ring-amber-300">
                  <Fingerprint className="size-4 text-neutral-500" />
                  <input
                    type="text"
                    name="dni"
                    value={form.dni}
                ...
                {form.dni && !validDni(form.dni) && (
                  <p className="mt-1 text-xs text-rose-600">Ingresá un DNI válido (7–9 dígitos).</p>
                )}
              </div>

              {/* Apellido */}
              <div>
                <label className="block text-sm font-medium text-neutral-800 mb-1">Apellido</label>
                <div className="flex items-center gap-2 rounded-xl border border-neutral-300 bg-white px-3 py-2 focus-within:ring-2 ring-amber-300">
                  <User className="size-4 text-neutral-500" />
                  <input
                    type="text"
