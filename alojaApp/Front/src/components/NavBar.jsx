  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // nuevo: resolver role desde diferentes formas de respuesta
        const j = await r.json().catch(() => ({}));
        const roleRaw =
          j?.data?.role ?? j?.role ?? j?.rol ?? j?.data?.rol ?? null;

        let role = typeof roleRaw === "string" ? roleRaw.toLowerCase() : "";
        // fallback si viene por id_rol
        if (!role) {
          const idRol = Number(j?.data?.id_rol ?? j?.id_rol ?? NaN);
          if (idRol === 5) role = "huesped";
          if (idRol === 6) role = "anfitrion";
        }
